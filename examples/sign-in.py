...
async def step_handling(_, msg: types.Message):
    await msg.reply("🔖 What's your username? (cancel with /cancel)")
...
async def get_password(_, msg: types.Message, username: str = None):
    ...
    if username == VALID_USERNAME and password == VALID_PASSWORD:
        await msg.reply("Successfuly signed in.")
        return

    await msg.reply("Invalid password or username! (send /step to try again)")
...
async def ask_handling(_, msg: types.Message):
    ...
    await msg.reply("Invalid password or username! (send /ask to try again)")
